...
- [ ] **`sessaocompra` (profile `web`) como árbitro ainda não está ligada aos outros serviços.** `SessaoCompraController.iniciarPagamento` e trechos de `atualizarEstadoCompra` têm TODOs explícitos para as chamadas que fariam o papel de árbitro de fato (ver comentários no próprio controller). Desenho de como fechar isso (endpoints incrementais, gate de completude, SAGA estendida) em [purchase-flow-design.md](purchase-flow-design.md).
- [ ] **Sem id de correlação na mensagem da SAGA.** A mensagem hoje só carrega `tipo` + o que o handler colocar. Antes de implementar os handlers de negócio, vale decidir que campo identifica a sessão de compra/reserva em toda a cadeia. Fica mais concreto com [purchase-flow-design.md](purchase-flow-design.md): cada handler precisa desse id pra saber qual linha local afetar.
